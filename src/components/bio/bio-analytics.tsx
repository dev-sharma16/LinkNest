...
import type { BioAnalytics } from "@/server/bio-analytics";
...
export function BioAnalytics() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["bio-analytics", range],
    queryFn: () => fetchJson<BioAnalytics>(`/api/bio/analytics?range=${range}`),
  });
  ...
  return (
    <div className="grid gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Bio analytics</h1>
          <p className="text-sm text-muted-foreground">
            Engagement across your public page.
          </p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard icon={Eye} label="Page views" value={data?.totalViews} loading={isLoading} />
        <StatCard icon={MousePointerClick} label="Clicks" value={data?.totalClicks} loading={isLoading} />
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Most clicked blocks on your page</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <div className="grid gap-2">
              {(data?.topBlocks ?? []).map((b, i) => (
                <div key={b.id} className="flex items-center justify-between rounded-lg px-2 py-2 hover:bg-muted/60">
              ...
              {(data?.recentActivity ?? []).slice(0, 10).map((a) => (
                <div key={a.id} className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground">
                    {a.eventType === "view" ? "Page view" : "Block click"}
                    {a.country ? ` · ${a.country}` : ""}
                  </span>

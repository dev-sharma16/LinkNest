...
export default async function BioPage({
  ...
  return (
    <main
      ...
      className="min-h-dvh px-4 py-24 text-foreground"
    >
      ...
      >
        <div className="grid gap-12 md:grid-cols-2 md:items-center">
          {/* Left: Profile info */}
          <div className="flex flex-col items-center md:items-start" style={{ textAlign: theme.alignment === "center" ? "left" : theme.alignment }}>
            {profile.avatar ? (
              <div className="bio-animate" style={{ animation: "bio-rise 0.5s ease-out both" }}>
                <img
                  ...
                  width={104}
                  height={104}
                  className="rounded-[16px] border object-cover transition-transform duration-300 hover:scale-105 md:mx-0"
                  style={{
            ...
            <h1
              className="bio-animate mt-6 text-4xl font-bold tracking-tight md:text-5xl"
              style={{
                fontFamily: "var(--font-serif)",
                fontSize: "calc(var(--bio-font-size) + 20px)",
                letterSpacing: "-1px",
            ...
            {profile.bio ? (
              <p className="bio-animate mt-4 text-lg leading-relaxed opacity-80" style={{ animation: "bio-rise 0.5s ease-out 0.16s both" }}>
                {profile.bio}
              </p>
            ) : null}

            {profile.location ? (
              <p className="bio-animate mt-2 text-sm opacity-60" style={{ animation: "bio-rise 0.5s ease-out 0.2s both" }}>
                {profile.location}
              </p>
            ) : null}
            ...
            {profile.website ? (
              <a
                ...
                className="bio-animate mt-2 inline-block text-sm underline opacity-70 transition-opacity hover:opacity-100"
                style={{ color: "var(--bio-primary)", animation: "bio-rise 0.5s ease-out 0.24s both" }}
              >
            ...
            {profile.socials.length > 0 ? (
              <div
                className="bio-animate mt-6 flex flex-wrap items-center gap-2"
                style={{ animation: "bio-rise 0.5s ease-out 0.28s both" }}
              >
          ...
          {profile.avatar ? (
            <div className="hidden md:block" style={{ animation: "bio-rise 0.5s ease-out 0.32s both" }}>
              <img
                src={profile.avatar}
                alt={profile.displayName}
                className="w-full rounded-[16px] object-cover"
                style={{
                  border: "1px solid color-mix(in srgb, var(--bio-primary) 20%, transparent)",
                }}
              />
            </div>
          ) : null}
      ...
      <footer className="mx-auto mt-24 w-full max-w-5xl text-center text-xs opacity-40">
        Made with LinkNest
      </footer>
    </main>
  );
